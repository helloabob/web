...
Slider.prototype = {
  //容器对象，滑块
  initialize: function(container, bar,barup,bardown,mask, options) {
	
	this.Bar = _$(bar);
	this.Container = _$(container);
	this.BarUp=_$(barup);//向上滑动箭头
	this.BarDown=_$(bardown);//向下滑动箭头
	this.Mask=_$(mask);//向下滑动箭头
	this._timer = null;//自动滑移的定时器
	this._ondrag = false;//解决ie的click问题
	this.BarUp.className=this.BarUp.className+" arrow-up-disable";//初始状态向上按钮显示不能点
	this.Mask.style.left="0px";//用遮罩屏蔽向上按钮
	...
	//点击向上按钮
	addEventHandler(this.BarDown, "click", BindAsEventListener(this, function(e){ this.UpDownCtrl(e,0);}));
	//点击向下按钮
	addEventHandler(this.BarUp, "click", BindAsEventListener(this, function(e){ this.UpDownCtrl(e,1);;}));
  ...
  Move: function() {
	...
	if(percent > 0){
		...
		if(!this._IsMin){ this.onMin(); this._IsMin = true;this.BarUp.className=this.BarUp.className+" arrow-up-disable";this.Mask.style.left="0px";}
	}
	//最大值判断
	if(percent < 1){
		this._IsMax = false;
	}else{
		if(!this._IsMax){ this.onMax(); this._IsMax = true;this.BarDown.className=this.BarDown.className+" arrow-down-disable";this.Mask.style.left="43px";}
	}
	//中间值判断
	if(percent > 0 && percent < 1){
		if(!this._IsMid){ this.onMid(); this._IsMid = true;this.BarUp.className="arrow-up";this.BarDown.className="arrow-down";;this.Mask.style.left="-60px"; }
	}else{
		this._IsMid = false;
	}
  },
  //向下向上控制
  UpDownCtrl: function(e,flag) {
	if(this.KeySpeed){
